...
- [ ] **Code Execution Tool da Anthropic pra verificação de cálculos** — Hoje o pipeline gera perguntas com cálculos numéricos via Sonnet 4.6 fazendo aritmética "de cabeça" (pattern matching). Acerta a maioria dos cálculos simples (1-2 etapas) mas erra ~10-20% em multi-etapa, derivadas/integrais, probabilidade, conversões de unidade. **Solução**: integrar a tool nativa `code_execution` da Anthropic — sandbox Python real (NumPy, sympy, scipy) que roda no servidor da Anthropic. Modelo decide quando usar via tool_use; resultado garantido correto. Refactor médio em `claude.service.ts:complete()` pra aceitar tools + loop processando tool_use até stop_reason='end_turn' (~1.5h). Prompt da geração instrui usar a tool quando pergunta envolver cálculo (~30min). Custo: +$0.15 por quiz com cálculos (overhead da tool). Latência: +1-2s por pergunta com cálculo. Pedro identificou essa fraqueza em 2026-05-05; importante pra quiz de exatas (cálculo II, álgebra, probabilidade) virar confiável. Ver discussão no histórico — Pedro mencionou: "claude oficial usa essa tool pra contar e estimar com pattern matching dá errado com frequência; com Python rodando NumPy é resposta exata". Estimativa: ~4h, v0.9.x ou v0.10.
